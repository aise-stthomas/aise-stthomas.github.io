...
The governing constraint on this syllabus is in `course-thesis.md`: **every topic must
...
`projects.md` for the account setup and the budget rules.
...
for each step are published in `rubric.md`.
...
on a broken base. Full specs, the scaffold contents, and the budgets: `projects.md`.

---

## Schedule

Reading lists the target book chapter (see `../book/`) and, until those chapters are
...
| Projects (4) | 40% | 10 each. Pairs. See `projects.md`. |
...
Full specs in `projects.md`. Each project: what you replace in the scaffold, a one-page
...
budget in days, and an exhausted budget deactivates the lab. See `projects.md`.

---

## Primary texts

No required purchase. The target text is the book under development in `../book/`;
